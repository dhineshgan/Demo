...
resource "azurerm_linux_web_app" "fe-webapp" {
  ...
  site_config { 
    ...
    ip_restriction {
        ip_address = "80.43.15.186/32"
        priority = 100
        name = "Front end Access restriction"
    }
  ...
  app_settings = {
    ...
    "APPINSIGHTS_INSTRUMENTATIONKEY"                  = azurerm_application_insights.dg-appinsights.instrumentation_key
  ...
  depends_on = [
    azurerm_service_plan.fe-asp,azurerm_application_insights.dg-appinsights
  ]
}
